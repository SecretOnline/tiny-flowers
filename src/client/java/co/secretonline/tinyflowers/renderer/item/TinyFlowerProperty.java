...
public record TinyFlowerProperty() implements SelectItemModelProperty<TinyFlowerComponent> {
	...
	public @Nullable TinyFlowerComponent get(ItemStack itemStack, @Nullable ClientLevel clientLevel,
			@Nullable LivingEntity entity, int i, ItemDisplayContext ctx) {
		return itemStack.get(ModComponents.TINY_FLOWER);
	}

	@Override
	public Type<? extends SelectItemModelProperty<TinyFlowerComponent>, TinyFlowerComponent> type() {
		return TYPE;
	}

	@Override
	public Codec<TinyFlowerComponent> valueCodec() {
		return VALUE_CODEC;
	}
